...
|postmaster_at_verify | boolean | Request an email with a verification link to be sent to the sending domain's postmaster@ mailbox. | no | Not SparkPost Enterprise |
|abuse_at_verify | boolean | Request an email with a verification link to be sent to the sending domain's abuse@ mailbox. | no | Not SparkPost Enterprise|
|postmaster_at_token | string | A token retrieved from the verification link contained in the postmaster@ verification email. | no | Not SparkPost Enterprise |
|abuse_at_token | string | A token retrieved from the verification link contained in the abuse@ verification email. | no | Not SparkPost Enterprise |
...
**[SparkPost Enterprise API only](https://www.sparkpost.com/enterprise-email/):** Verification by methods other than DKIM are not applicable to Enterprise accounts and should not be used 
