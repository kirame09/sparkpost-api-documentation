...
|return_path | string | **[SparkPost Enterprise API only](https://www.sparkpost.com/enterprise-email/)**: email to use for envelope FROM | required for email transmissions | The local part of the return_path address must start with the string `bounce` eg bounce@mail.company.com or bounce-123abd@mail.company.com |
