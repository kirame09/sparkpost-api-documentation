...
**Deprecation Notice:** The `bindings` and `binding_groups` query parmeters have been deprecated. Please use the `sending_ips` and `ip_pools` query parameters instead. For SparkPost Enterprise `ip_pools` is the same as the `binding` field used for tranmissions
